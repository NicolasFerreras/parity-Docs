...
|---|---|---|---|
| POST | `/orders/upload` (stdlib, streaming) | Recibe Excel (≤10MB) o PDF (tope a definir) por carga manual, guarda en Storage, crea `orders` + `order_lines` en `revisar` | Dispara pipeline: Excel→excelize; PDF→pdfcpu+texto+LLM. Sin ingesta automática en MVP |
| GET | `/orders/:id` | Orden + líneas con estados y candidatos | Incluye origen del match y candidatos por línea |
...
| DELETE | `/orders/:id/lines/:lineId` | Quita línea | Log de auditoría |
...
| GET | `/jobs/:id` | Estado del job async (polling "validando con IA…") | pendiente, procesando, listo o fallido |
| GET | `/catalog?q=&limit=&offset=` | Busca artículos (texto aproximado + trigramas, para el dropdown con buscador) | Ordenamiento de candidatos: calibra equipo |
